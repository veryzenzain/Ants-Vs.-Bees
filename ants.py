...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        #Reduce fire ants health
        place = self.place
        super().reduce_health(amount) #Calling inherited method (Insect)
        
        #Reduce bees health
        all_bees = list(place.bees) #get list of all bees in our place!
        for bee in all_bees:
            if self.health <= 0: #ant is dead
                bee.reduce_health(amount + self.damage) #deal amount + damage
            if self.health > 0: #ant is still alive
                bee.reduce_health(amount) #deal amount
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, gamestate):
        ...
        all_bees = list(self.place.bees) #get list of all bees in our place!
        for bee in all_bees:
...
class QueenAnt(ScubaThrower):  
    ...
    def action(self, gamestate):
        ...
        while place.exit:
            place = place.exit
            if place.ant:
                super().double(place.ant)
                if place.ant.is_container and place.ant.ant_contained:
                    super().double(place.ant.ant_contained)
